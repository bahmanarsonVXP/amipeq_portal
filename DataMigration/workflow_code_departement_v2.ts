export const main = async (params: {
  ...
  function extractDepartement(cp: string) {
    ...
    // DOM-TOM
    if (cpClean.startsWith('971')) return { numero: '971', code: '971 - Guadeloupe' };
    if (cpClean.startsWith('972')) return { numero: '972', code: '972 - Martinique' };
    if (cpClean.startsWith('973')) return { numero: '973', code: '973 - Guyane' };
    if (cpClean.startsWith('974')) return { numero: '974', code: '974 - La Réunion' };
    if (cpClean.startsWith('976')) return { numero: '976', code: '976 - Mayotte' };

    // Corse
    if (cpClean.startsWith('20')) {
      const num = parseInt(cpClean);
      if (num < 20200) return { numero: '2A', code: '2A - Corse-du-Sud' };
      return { numero: '2B', code: '2B - Haute-Corse' };
    }
